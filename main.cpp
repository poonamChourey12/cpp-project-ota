...
public:
    ...
    [[nodiscard]] DownloadResult download_chunk(
        ...
        // Simulate network timeout using init-statement
        if (std::uniform_real_distribution timeout_dis(0.0, 1.0); timeout_dis(rng_) < failure_rate) {
...
public:
    // Use in-class initializer instead of constructor initializer list
    ...
    void log_event(const std::string& event) {
        ...
        // Use std::chrono instead of localtime
        ...
        auto local_tod = local_time - local_days;
        
        auto hours = std::chrono::duration_cast<std::chrono::hours>(local_tod);
        auto minutes = std::chrono::duration_cast<std::chrono::minutes>(local_tod - hours);
        auto seconds = std::chrono::duration_cast<std::chrono::seconds>(local_tod - hours - minutes);
        
        update_log_.push_back(std::format("[{:%Y-%m-%d} {:02}:{:02}:{:02}] {}", 
            local_days, hours.count(), minutes.count(), seconds.count(), event));
...
private:
    ...
    // Use transparent equality and custom hasher
